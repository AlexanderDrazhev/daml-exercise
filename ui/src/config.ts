...
export const userManagement: UserManagement = {
  ...
  publicParty: async (loginName, ledger: Ledger) => {
    ...
    const readAsRights: CanReadAs[] = rights.filter((right): right is CanReadAs => right.type === "CanReadAs");
...
export const insecure: Insecure = (() => {
  const ledgerId: string = import.meta.env.VITE_LEDGER_ID ?? "create-daml-app-sandbox";
  return {
